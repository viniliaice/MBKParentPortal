...
function FillBlankActivity({ activity, submitted, onSubmit }: {
  ...
  return (
    <View>
      <View style={[styles.fillSentence, { backgroundColor: c.muted }]}>
        {parts.map((part, i) => (
          <React.Fragment key={i}>
            <Text style={[styles.fillText, { color: c.foreground }]}>{part}</Text>
            {i < parts.length - 1 && (
              <View style={[styles.blankBox, { borderColor: c.border }, selected && { borderColor: '#3D5AFE', backgroundColor: 'rgba(61,90,254,0.15)' }]}>
        ...
        {activity.options.map(opt => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={[styles.chipText, { color: c.foreground }]}>{opt}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
function DragOrderActivity({ activity, submitted, onSubmit }: {
  ...
  return (
    <View>
      <Text style={[styles.subLabel, { color: c.mutedForeground }]}>TAP IN THE CORRECT ORDER</Text>
      <View style={[styles.orderAnswerArea, { borderColor: c.border }]}>
        {order.length === 0 ? (
          <Text style={[styles.placeholderText, { color: c.mutedForeground }]}>Tap items below to add them…</Text>
        ) : (
      ...
      <View style={styles.chipRow}>
        {remaining.map(item => (
          <TouchableOpacity key={item} style={[styles.chipBtn, { borderColor: c.border, backgroundColor: c.muted }]} onPress={() => addItem(item)} activeOpacity={0.7}>
...
function MatchPairsActivity({ activity, submitted, onSubmit }: {
  ...
  return (
    <View>
      <Text style={[styles.subLabel, { color: c.mutedForeground }]}>TAP TO MATCH</Text>
        ...
        <View style={styles.matchCol}>
          {leftItems.map(item => {
            ...
            return (
              <TouchableOpacity
                key={item}
                style={[styles.matchChip, { backgroundColor: c.muted, borderColor: c.border },
...
function NumberLineActivity({ activity, submitted, onSubmit }: {
  ...
  return (
    <View>
      <Text style={[styles.subLabel, { color: c.mutedForeground }]}>TAP THE CORRECT NUMBER</Text>
      <View style={styles.numberLineContainer}>
        <View style={[styles.numberLineTrack, { backgroundColor: c.border }]} />
        <View style={styles.numberLineNumbers}>
          {numbers.map(n => {
            let bg = c.muted;
            let textColor = c.mutedForeground;
      ...
      {['True', 'False'].map(opt => {
        ...
        return (
          ...
          >
            ...
            <Text style={[styles.tfBtnText, { color: c.foreground }]}>{opt}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
...
function WritingActivity({ activity, submitted, onSubmit }: {
  ...
  return (
    <View>
      <Text style={[styles.subLabel, { color: c.mutedForeground }]}>SELECT THE CORRECT SPELLING</Text>
      <View style={styles.optionGrid}>
        {activity.options.map(opt => {
          const isSelected = selected === opt;
          const correct = activity.correctAnswer;
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={[styles.optionText, { color: c.foreground, fontFamily: 'monospace', fontSize: 17 }]}>{opt}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
    ...
    <View style={{ marginTop: 12 }}>
      {!show ? (
        <TouchableOpacity style={styles.hintBtn} onPress={() => setShow(true)} activeOpacity={0.7}>
          ...
          <Text style={[styles.hintBtnText, { color: '#F59E0B' }]}>Need a hint?</Text>
        </TouchableOpacity>
      ) : (
        <View style={styles.hintBox}>
          <Ionicons name="bulb" size={16} color="#F59E0B" />
          <Text style={[styles.hintText, { color: c.mutedForeground }]}>{hint}</Text>
        </View>
      )}
    </View>
  );
